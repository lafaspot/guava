 ...
 *
 * <p>
 * This tests {@code foo.equals(foo)}, {@code foo.equals(null)}, and a few other
 * operations.
 *
 * <p>
 * For more extensive testing, add multiple equality groups. Each group should
 * contain objects that are equal to each other but unequal to the objects in
 * any other group. For example:
 *
 * <pre>
 * new EqualsTester().addEqualityGroup(new User("page"), new User("page")).addEqualityGroup(new User("sergey"))
 * 		.testEquals();
 * </pre>
 *
 * <p>
 ...
 *
 * <p>
 * When a test fails, the error message labels the objects involved in the
 * failed comparison as follows:
 *
 * <ul>
 * <li>"{@code [group }<i>i</i>{@code , item }<i>j</i>{@code ]}" refers to the
 * <i>j</i><sup>th</sup> item in the <i>i</i><sup>th</sup> equality group, where
 * both equality groups and the items within equality groups are numbered
 * starting from 1. When either a constructor argument or an equal object is
 * provided, that becomes group 1.
 * </ul>
 *
 * @author Jim McMaster
 * @author Jige Yu
...
public final class EqualsTester {
	...
	/**
	 * Adds {@code equalityGroup} with objects that are supposed to be equal to each
	...
	private void testItems() {
		for (Object item : equalityGroups) {
	...
	/**
	 * Class used to test whether equals() correctly handles an instance of an
	 * incompatible class. Since it is a private inner class, the invoker can never
	 * pass in an instance to the tester
	 */

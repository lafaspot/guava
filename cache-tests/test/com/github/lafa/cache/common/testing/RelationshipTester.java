...
final class RelationshipTester<T> {
	...
	/**
	 * A word about using {@link Equivalence}, which automatically checks for
	 * {@code null} and identical inputs: This sounds like it ought to be a problem
	 * here, since the goals of this class include testing that {@code equals()} is
	 * reflexive and is tolerant of {@code null}. However, there's no problem. The
	 * reason: {@link EqualsTester} tests {@code null} and identical inputs directly
	 * against {@code equals()} rather than through the {@code Equivalence}.
	 */
	...
	// TODO(cpovirk): should we reject null items, since the tests already check
	// null automatically?
	public RelationshipTester<T> addRelatedGroup(List<? extends T> group) {
		List<T> newList = new ArrayList<>(group);
		groups.add(newList);
		return this;
	}
	...
	public void test() {
		for (int groupNumber = 0; groupNumber < groups.size(); groupNumber++) {
			List<T> group = groups.get(groupNumber);
			for (int itemNumber = 0; itemNumber < group.size(); itemNumber++) {
				...
				for (int unrelatedGroupNumber = 0; unrelatedGroupNumber < groups.size(); unrelatedGroupNumber++) {
					if (groupNumber != unrelatedGroupNumber) {
						List<T> unrelatedGroup = groups.get(unrelatedGroupNumber);

...
abstract class DummyProxy {
	...
	/**
	 * Returns a new proxy for {@code interfaceType}. Proxies of the same interface
	 * are equal to each other if the {@link DummyProxy} instance that created the
	 * proxies are equal.
	 */
	final <T> T newProxy(TypeToken<T> interfaceType) {
		...
		System.out.println("interface classes =====> " + interfaceClasses);
		System.out.println("-----------------------------------------------");
		Object dummy = Proxy.newProxyInstance(interfaceClasses.iterator().next().getClassLoader(),
				interfaceClasses.toArray(new Class<?>[interfaceClasses.size()]), new DummyHandler(interfaceType));
	...
	private class DummyHandler extends AbstractInvocationHandler implements Serializable {
		...
		// Since type variables aren't serializable, reduce the type down to raw type
		// before
		// serialization.

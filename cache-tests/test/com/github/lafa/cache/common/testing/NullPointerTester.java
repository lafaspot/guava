...
import com.github.lafa.cache.base.Converter;
 ...
 * {@link NullPointerException} or {@link UnsupportedOperationException}
 * whenever null is passed to a parameter that isn't annotated with
 * {@link javax.annotation.Nullable}, {@link javax.annotation.CheckForNull}, or
 * {@link org.checkerframework.checker.nullness.compatqual.NullableDecl}.
 *
 * <p>
 ...
 * expected exception is thrown. {@code
 * NullPointerTester} uses best effort to pick non-null default values for many
 * common JDK and Guava types, and also for interfaces and public classes that
 * have public parameter-less constructors. When the non-null default value for
 * a particular parameter type cannot be provided by {@code
 * NullPointerTester}, the caller can provide a custom non-null default value
 * for the parameter type via {@link #setDefault}.
 *
...
public final class NullPointerTester {
	...
	private final ClassToInstanceMap<Object> defaults = new ClassToInstanceMap<>();
	private final List<Member> ignoredMembers = new ArrayList<>();

	private ExceptionTypePolicy policy = ExceptionTypePolicy.NPE_OR_UOE;

	/**
	 * Sets a default value that can be used for any parameter of type {@code type}.
	 * Returns this object.
	 */
	 ...
	 * at least {@code
	 * minimalVisibility}, including those "inherited" from superclasses of the same
	 * package.
	 */
	...
	List<Method> getInstanceMethodsToTest(Class<?> c, Visibility minimalVisibility) {
		List<Method> builder = new ArrayList<>();
	 ...
	 * {@link UnsupportedOperationException} whenever <i>any</i> of its non-nullable
	 * parameters are null.
	 *
	 * @param instance
	 *            the instance to invoke {@code method} on, or null if
	...
	public void testConstructor(Constructor<?> ctor) {
		...
		checkArgument(Modifier.isStatic(declaringClass.getModifiers()) || declaringClass.getEnclosingClass() == null,
	...
	/**
	 * Verifies that {@code method} produces a {@link NullPointerException} or
	 * {@link UnsupportedOperationException} when the parameter in position
	 * {@code paramIndex} is null. If this parameter is marked nullable, this method
	 * does nothing.
	 *
	 * @param instance
	 *            the instance to invoke {@code method} on, or null if
	 *            {@code method} is static
	 */
	public void testMethodParameter(@NullableDecl final Object instance, final Method method, int paramIndex) {
	...
	public enum Visibility {
		...
		final Iterable<Method> getInstanceMethods(Class<?> cls) {
			ConcurrentMap<Signature, Method> map = new ConcurrentHashMap<>();
			for (Method method : getVisibleMethods(cls)) {
				if (!Invokable.from(method).isStatic()) {
		...
		private List<Method> getVisibleMethods(Class<?> cls) {
	...
	private static final class Signature {
		...
		Signature(String name, List<Class<?>> parameterTypes) {
			this.name = name;
	 ...
	 *
	 * @param instance
	 *            the instance to invoke {@code invokable} on, or null if
	...
	private void testParameter(Object instance, Invokable<?, ?> invokable, int paramIndex, Class<?> testedClass) {
		...
		try {
			...
			Assert.fail("No exception thrown for parameter at index " + paramIndex + " from " + invokable
			...
			AssertionFailedError error = new AssertionFailedError(String.format(
					"wrong exception thrown from %s when passing null to %s parameter at index %s.%n"
							+ "Full parameters: %s%n" + "Actual exception message: %s",
					invokable, invokable.getParameters().get(paramIndex).getType(), paramIndex, Arrays.toString(params),
					cause));
	...
	private Object[] buildParamList(Invokable<?, ?> invokable, int indexOfParamToSetToNull) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (i != indexOfParamToSetToNull) {
				...
				Assert.assertTrue(
						"Can't find or create a sample instance for type '" + param.getType()
	...
	private <F, T> Converter<F, T> defaultConverter(final TypeToken<F> convertFromType,
			final TypeToken<T> convertToType) {
	...
	private static final Set<String> NULLABLE_ANNOTATIONS = new HashSet<>(Arrays.asList("javax.annotation.CheckForNull",
			"javax.annotation.Nullable", "org.checkerframework.checker.nullness.compatqual.NullableDecl"));
	 ...
	 *
	 * <p>
	 * The documentation for {@link Object#equals} says it should accept null, so
	 * don't require an explicit {@code @NullableDecl} annotation (see
	 * <a href="https://github.com/google/guava/issues/1819">#1819</a>).
	 *
	 * <p>
	 * It is not necessary to consider visibility, return type, or type parameter
	 * declarations. The declaration of a method with the same name and formal
	 * parameters as {@link Object#equals} that is not public and boolean-returning,
	 * or that declares any type parameters, would be rejected at compile-time.
	 */
	...
	private enum ExceptionTypePolicy {
		 ...
		 * {@link IllegalArgumentException}, or {@link UnsupportedOperationException}.
		 */
		NPE_IAE_OR_UOE() {
			@Override
			public boolean isExpectedType(Throwable cause) {
				return cause instanceof NullPointerException || cause instanceof IllegalArgumentException

 ...
 *
 * <p>
 * Use this class to test code triggered by <em>finalization</em>, that is, one
 * of the following actions taken by the java garbage collection system:
 *
 * <ul>
 ...
 * {@link java.lang.System#gc()} to cause finalization to happen. However, a
 * call to {@code System.gc()} is specified to be no more than a hint, so this
 * technique may fail at the whim of the JDK implementation, for example if a
 * user specified the JVM flag {@code -XX:+DisableExplicitGC}. But in practice,
 * it works very well for ordinary tests.
 *
 * <p>
 * Failure of the expected event to occur within an implementation-defined
 ...
 *
 * <p>
 * Even if your non-test code does not use finalization, you can use this class
 * to test for leaks, by ensuring that objects are no longer strongly
 * referenced:
 *
 * <pre>
 * {@code
 ...
 *
 * <p>
 * This class cannot currently be used to test soft references, since this class
 * does not try to create the memory pressure required to cause soft references
 * to be cleared.
 *
 * <p>
 * This class only provides testing utilities. It is not designed for direct use
 * in production or for benchmarking.
 *
...
public final class GcFinalization {
	 ...
	 * Unless we have a gigantic heap, in which case we scale by heap size.
	 */
	private static long timeoutSeconds() {
		// This class can make no hard guarantees. The methods in this class are
		// inherently flaky, but
		// we try hard to make them robust in practice. We could additionally try to add
		// in a system
		// load timeout multiplier. Or we could try to use a CPU time bound instead of
		// wall clock time
		// bound. But these ideas are harder to implement. We do not try to detect or
		// handle a
	...
	/**
	 * Waits until the given future {@linkplain Future#isDone is done}, invoking the
	 * garbage collector as necessary to try to ensure that this will happen.
	 *
	...
	/**
	 * Waits until the given predicate returns true, invoking the garbage collector
	 ...
	 * down} to zero, invoking the garbage collector as necessary to try to ensure
	 ...
	 * garbage collector when performing a full collection in response to
	 * {@link System#gc()}:
	 *
	 * <ul>
	 * <li>invoking the {@code finalize} methods of unreachable objects
	 ...
	 * complete. Ensures that at least one weak reference has been cleared and one
	 * {@code finalize} method has been run before this method returns. This method
	 * may be useful when testing the garbage collection mechanism itself, or
	 * inhibiting a spontaneous GC initiation in subsequent code.
	 *
	 * <p>
	 * In contrast, a plain call to {@link java.lang.System#gc()} does not ensure
	 * finalization processing and may run concurrently, for example, if the JVM
	 * flag {@code
	 * -XX:+ExplicitGCInvokesConcurrent} is used.
	 *
	 * <p>
	 * Whenever possible, it is preferable to test directly for some observable
	 * change resulting from GC, as with {@link #awaitClear}. Because there are no
	 * guarantees for the order of GC finalization processing, there may still be
	 * some unfinished work for the GC to do after this method returns.
	 *
	 * <p>
	 * This method does not create any memory pressure as would be required to cause
	 * soft references to be processed.
	 *
	 * @throws RuntimeException
	 *             if timed out or interrupted while waiting
	 * @since 12.0

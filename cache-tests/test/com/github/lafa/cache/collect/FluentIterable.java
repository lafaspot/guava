...
public abstract class FluentIterable<E> implements Iterable<E> {
	...
	// To avoid a self retain cycle under j2objc, we store Optional.absent() instead
	// of
	...
	FluentIterable(Iterable<E> iterable) {
		...
		this.iterableDelegate = (iterable == null) ? Optional.empty() : Optional.of(iterable);
	}

	private Iterable<E> getDelegate() {
		return iterableDelegate.orElse(this);
	}
	 ...
	 * {@code iterable} is a {@link Collection}; {@link Streams#stream(Iterable)}
	 * otherwise.
	 */
	 ...
	 * After {@code
	 * remove()} is called, subsequent cycles omit the removed element, which is no
	 * longer in this fluent iterable. The iterator's {@code hasNext()} method
	 * returns {@code true} until this fluent iterable is empty.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> if the source iterable has only a single
	 * element {@code
	 * e}, use {@code Stream.generate(() -> e)}. Otherwise, collect your stream into
	 ...
	 * fluent iterable, followed by those of {@code other}. The iterators are not
	 ...
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.filter(type::isInstance).map(type::cast)}. This does perform a
	 * little more work than necessary, so another option is to insert an unchecked
	 * cast at some later point:
	 *
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Stream#flatMap} (using a function
	 * that produces streams, not iterables).
	 *
	 ...
	 * iterable. If the iterable is empty, {@code Optional.absent()} is returned. If
	 * the underlying {@code iterable} is a {@link List} with
	 * {@link java.util.RandomAccess} support, then this operation is guaranteed to
	 * be {@code O(1)}.
	 *
	 * <p>
	 ...
	 * The returned fluent iterable's iterator supports {@code
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> pass {@link ImmutableList#toImmutableList}
	 * to {@code
	 * stream.collect()}.
	 *
	 * @throws NullPointerException
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> pass {@link ImmutableSet#toImmutableSet} to
	 * {@code
	 * stream.collect()}.
	 *
	 * @throws NullPointerException
	 ...
	 * instance of that key and, if it is, which result will be mapped to that key
	 * in the returned map.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 ...
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.collect(Collectors.groupingBy(keyFunction))} behaves similarly,
	 * but returns a mutable {@code Map<K, List<E>>} instead, and may not preserve
	 ...
	 * {@code values}, indexed by keys derived from those values. In other words,
	 * each input value produces an entry in the map whose key is the result of
	 * applying {@code keyFunction} to that value. These entries appear in the same
	 * order as they appeared in this fluent iterable. Example usage:
	 *
	 * <pre>
	 * {@code
	 ...
	 *         value
	 * @throws IllegalArgumentException
	 *             if {@code keyFunction} produces the same key for more than one
	 ...
	 * {@code
	 * stream.toArray()}; if {@code type} is a class literal such as
	 * {@code MyType.class}, use {@code
	 * stream.toArray(MyType[]::new)}. Otherwise use
	 * {@code stream.toArray( len -> (E[])
	 ...
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.forEachOrdered(collection::add)} or {@code
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@code joiner.join(stream.iterator())}, or,
	 * if you are not using any optional {@code Joiner} features, {@code
	 ...
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.skip(position).findFirst().get()} (but note that this throws
	 * different exception types, and throws an exception if {@code null} would be
	 * returned).
	 *
	 * @param position
	 ...
	 * (ideally not going through {@code FluentIterable} at all), the more

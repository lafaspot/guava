...
package com.github.lafa.cache.collect.testing.features;
...
public enum CollectionFeature implements Feature<Collection> {
	/**
	 * The collection must not throw {@code NullPointerException} on calls such as
	 * {@code
	 * contains(null)} or {@code remove(null)}, but instead must return a simple
	 * {@code false}.
	 */
	ALLOWS_NULL_QUERIES, ALLOWS_NULL_VALUES(ALLOWS_NULL_QUERIES),

	/**
	 * Indicates that a collection disallows certain elements (other than
	 * {@code null}, whose validity as an element is indicated by the presence or
	 * absence of {@link #ALLOWS_NULL_VALUES}). From the documentation for
	 * {@link Collection}:
	 *
	 * <blockquote>
	 *
	...
	/**
	 * Indicates that a collection has a well-defined ordering of its elements. The
	 * ordering may depend on the element values, such as a {@link SortedSet}, or on
	 * the insertion ordering, such as a {@link LinkedHashSet}. All list tests and
	 * sorted-collection tests automatically specify this feature.
	 */
	KNOWN_ORDER,

	/**
	 * Indicates that a collection has a different {@link Object#toString}
	 * representation than most collections. If not specified, the collection tests
	 * will examine the value returned by {@link Object#toString}.
	 */
	NON_STANDARD_TOSTRING,

	/**
	 * Indicates that the constructor or factory method of a collection, usually an
	 * immutable set, throws an {@link IllegalArgumentException} when presented with
	 * duplicate elements instead of collapsing them to a single element or

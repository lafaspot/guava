...
import static com.github.lafa.cache.collect.testing.Helpers.assertEqualIgnoringOrder;
import static com.github.lafa.cache.collect.testing.Helpers.assertEqualInOrder;
import static com.github.lafa.cache.collect.testing.Platform.format;
...
public final class SpliteratorTester<E> {
	...
	public interface Ordered {
		/**
		 * Attests that the expected values must not just be present but must be present
	...
	enum SpliteratorDecompositionStrategy {
		...
		MAXIMUM_SPLIT {
			@Override
			<E> void forEach(Spliterator<E> spliterator, Consumer<? super E> consumer) {
				for (Spliterator<E> prefix = trySplitTestingSize(
						spliterator); prefix != null; prefix = trySplitTestingSize(spliterator)) {
	...
	private static <E> Spliterator<E> trySplitTestingSize(Spliterator<E> spliterator) {
		...
		if (spliterator.estimateSize() > originalSize) {
			fail(format("estimated size of spliterator after trySplit (%s) is larger than original size (%s)",
					spliterator.estimateSize(), originalSize));
		}
		if (trySplit != null) {
			if (trySplit.estimateSize() > originalSize) {
				fail(format("estimated size of trySplit result (%s) is larger than original size (%s)",
						trySplit.estimateSize(), originalSize));
			}
		}
		if (subsized) {
			if (trySplit != null) {
				assertEquals("sum of estimated sizes of trySplit and original spliterator after trySplit", originalSize,
						trySplit.estimateSize() + spliterator.estimateSize());
			} else {
				assertEquals("estimated size of spliterator after failed trySplit", originalSize,
	...
	public final Ordered expect(Iterable<?> elements) {
		...
		for (SpliteratorDecompositionStrategy strategy : EnumSet.allOf(SpliteratorDecompositionStrategy.class)) {
			...
			if ((characteristics & Spliterator.SORTED) != 0) {
				...
				// assertTrue(new Compartor(comparator).isOrdered(resultsForStrategy));
			}
			if ((characteristics & Spliterator.SIZED) != 0) {
				assertEquals((int) estimatedSize, resultsForStrategy.size());
			}
		...
		return new Ordered() {
			@Override
			public void inOrder() {
				resultsForAllStrategies.forEach(resultsForStrategy -> assertEqualInOrder(elements, resultsForStrategy));
			}
		};
	}
}

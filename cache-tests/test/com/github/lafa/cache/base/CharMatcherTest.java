...
import static com.github.lafa.cache.base.CharMatcher.inRange;
import static com.github.lafa.cache.base.CharMatcher.is;
import static com.github.lafa.cache.base.CharMatcher.isNot;
...
public class CharMatcherTest extends TestCase {
	...
	/**
	 * Checks that expected is equals to out, and further, if in is equals to
	 * expected, then out is successfully optimized to be identical to in, i.e. that
	 * "in" is simply returned.
	 */
	...
	static void checkExactMatches(CharMatcher m, char[] chars) {
		Set<Character> positive = new HashSet<Character>(chars.length);
		for (char c : chars) {
			positive.add(c);
		}
		for (int c = 0; c <= Character.MAX_VALUE; c++) {

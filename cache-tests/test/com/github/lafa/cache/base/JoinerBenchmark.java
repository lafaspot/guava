...
import com.github.lafa.cache.base.Strings;

import java.util.Arrays;
import java.util.Iterator;

/**
 * Benchmarks {@link Joiner} against some common implementations of
 * delimiter-based string joining.
 *
...
public class JoinerBenchmark {
	...
	/**
	 * Mimics what the {@link Joiner} class does internally when no extra options
	 * like ignoring {@code
	...
	/**
	 * Only appends delimiter if the accumulated string is non-empty. Note: this
	 * isn't a candidate implementation for Joiner since it fails on leading empty
	 * components.
	 */
	@Benchmark
	...
	/**
	 * Similar to the above, but keeps a boolean flag rather than checking for the
	 * string accumulated so far being empty. As a result, it does not have the
	 * above-mentioned bug.
	 */
	@Benchmark
	...
	/**
	 * Starts with an empty delimiter and changes to the desired value at the end of
	 * the iteration.
	 */
	@Benchmark
	...
	/**
	 * Always append the delimiter after the component, and in the very end shortens
	 * the buffer to get rid of the extra trailing delimiter.
	 */
	@Benchmark

...
import static com.github.lafa.cache.lrucache.CacheBuilder.NULL_TICKER;
import static com.github.lafa.cache.lrucache.LocalCache.DISCARDING_QUEUE;
import static com.github.lafa.cache.lrucache.LocalCache.DRAIN_THRESHOLD;
import static com.github.lafa.cache.lrucache.LocalCache.nullEntry;
import static com.github.lafa.cache.lrucache.LocalCache.unset;
...
import com.github.lafa.cache.collect.testing.MapTestSuiteBuilder;
import com.github.lafa.cache.collect.testing.TestStringMapGenerator;
...
import com.github.lafa.cache.common.testing.TestLogHandler;
import com.github.lafa.cache.lrucache.LocalCache.EntryFactory;
...
public class LocalCacheTest extends TestCase {
	...
	private static void checkConcurrencyLevel(int concurrencyLevel, int segmentCount) {
		...
		assertEquals(map.segments.length, segmentCount);
	}
	...
	public void testValues() {
		...
		assertTrue(map.values().removeAll(new HashSet<>(Arrays.asList("bar"))));
	...
	public void testComputeIfAbsent_RemovalListener() {
		...
		Cache<Object, Object> cache = CacheBuilder.newBuilder().removalListener(removalListener).build();
		cache.put("a", "b");
	...
	public void testSegmentReplaceValue() {
		LocalCache<Object, Object> map = makeLocalCache(
				createCacheBuilder().concurrencyLevel(1).expireAfterAccess(99999, SECONDS));
		Segment<Object, Object> segment = map.segments[0];
		// TODO(fry): check recency ordering
	...
	public void testExpand() {
		...
		Map<Object, Object> originalMap = new HashMap<>(map);
	...
	public void testPutCausesExpansion() {
		for (int count = 1; count <= 100; count++) {
			LocalCache<Object, Object> map = makeLocalCache(
					createCacheBuilder().concurrencyLevel(1).initialCapacity(1));
			Segment<Object, Object> segment = map.segments[0];
			assertEquals(1, segment.table.length());
	...
	public void testExpand_cleanup() {
		...
		for (int i = 1; i <= originalCount * 2; i *= 2) {
			...
			assertEquals(originalMap, new HashMap<>(map));
		}
	}
	...
	public void testRemoveEntry() {
		LocalCache<Object, Object> map = makeLocalCache(
				createCacheBuilder().concurrencyLevel(1).initialCapacity(1).maximumSize(SMALL_MAX_SIZE)
	...
	static <K, V> void checkAndDrainRecencyQueue(LocalCache<K, V> map, Segment<K, V> segment,
		...
		if (map.evictsBySize() || map.expiresAfterAccess()) {
			assertSameEntries(reads, new ArrayList<>(segment.recencyQueue));
		}
	...
	static <K, V> void checkEvictionQueues(LocalCache<K, V> map, Segment<K, V> segment,
		...
		if (map.evictsBySize() || map.expiresAfterAccess()) {
			assertSameEntries(readOrder, new ArrayList<>(segment.accessQueue));
		}
		if (map.expiresAfterWrite()) {
			assertSameEntries(writeOrder, new ArrayList<>(segment.writeQueue));
		}
	}
	...
	/**
	 * Returns an iterable containing all combinations of maximumSize,
	 * expireAfterAccess/Write, weakKeys and weak/softValues.
	 */
	private static Iterable<CacheBuilder<Object, Object>> allEntryTypeMakers() {
		List<CacheBuilder<Object, Object>> result = (List<CacheBuilder<Object, Object>>) allKeyValueStrengthMakers();
		for (CacheBuilder<Object, Object> builder : allKeyValueStrengthMakers()) {
	...
	/**
	 * Returns an iterable containing all combinations of maximumSize and
	 * expireAfterAccess/Write.
	 */
	static Iterable<CacheBuilder<Object, Object>> allEvictingMakers() {
		return Arrays.asList(createCacheBuilder().maximumSize(SMALL_MAX_SIZE),
	...
	private static Iterable<CacheBuilder<Object, Object>> allKeyValueStrengthMakers() {
		return new ArrayList<>(Arrays.asList(createCacheBuilder(), createCacheBuilder().weakValues(),
				createCacheBuilder().softValues(), createCacheBuilder().weakKeys(), createCacheBuilder().weakKeys().weakValues(),

...
import com.github.lafa.cache.reflect.TypeResolver.TypeVariableKey;
 ...
 *
 * <p>
 * Operations that are otherwise only available in {@link Class} are implemented
 * to support {@code Type}, for example {@link #isSubtypeOf}, {@link #isArray}
 * and {@link #getComponentType}. It also provides additional utilities such as
 ...
 * subclass. The following code is wrong because it only captures the
 * {@code <T>} type variable of the {@code listType()} method signature; while
 * {@code <String>} is lost in erasure:
 * 
 * <pre>
 * {
 * 	&#64;code
 ...
 * </pre>
 * 
 * <li>Capture a generic type with a (usually anonymous) subclass and resolve it
 * against a context class that knows what the type parameters are. For example:
 * 
 * <pre>
 * {@code
 ...
 *
 * <p>
 * {@code TypeToken} is serializable when no type variable is contained in the
 * type.
 *
 * <p>
 * Note to Guice users: {@code} TypeToken is similar to Guice's
 * {@code TypeLiteral} class except that it is serializable and offers numerous
...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable {
	...
	/**
	 * Constructs a new type token of {@code T} while resolving free type variables
	 * in the context of {@code declaringClass}.
	 *
	 * <p>
	 * Clients create an empty anonymous subclass. Doing so embeds the type
	 * parameter in the anonymous class's type hierarchy so we can reconstitute it
	 ...
	 * returned by {@link java.lang.reflect.Method#getGenericReturnType}, the raw
	 * type is what's returned by {@link java.lang.reflect.Method#getReturnType} of
	 * the same method object. Specifically:
	 *
	 * <ul>
	 ...
	 * <li>If {@code T} is a type variable or a wildcard type, the raw type of the
	 * first upper bound is returned. For example: {@code <X extends Foo> => Foo}.
	 * </ul>
	 */
	 ...
	 * {@code typeParam} are substituted by {@code typeArg}. For example, it can be
	 * used to construct {@code Map<K, V>} for any {@code K} and {@code V} type:
	 *
	 * <pre>
	 * {@code
	 ...
	 *            The parameter type
	 * @param typeParam
	 *            the parameter type variable
	 * @param typeArg
	 *            the actual type to substitute
	 */
	public final <X> TypeToken<T> where(TypeParameter<X> typeParam, TypeToken<X> typeArg) {
		final Map<TypeVariableKey, Type> map = new HashMap<>();
		map.put(new TypeResolver.TypeVariableKey(typeParam.typeVariable), typeArg.runtimeType);
		TypeResolver resolver = new TypeResolver().where(map);
	...
	/**
	 * Resolves the given {@code type} against the type context represented by this
	 * type. For example:
	 *
	 * <pre>
	 * {@code
	...
	/**
	 * Returns the generic superclass of this type or {@code null} if the type
	 * represents {@link Object} or an interface. This method is similar but
	 * different from {@link Class#getGenericSuperclass}. For example, {@code new
	 * TypeToken<StringArrayList>() {}.getGenericSuperclass()} will return
	 * {@code new TypeToken<ArrayList<String>>() {}}; while
	 ...
	 *
	 * <p>
	 * If this type is a type variable or wildcard, its first upper bound is
	 * examined and returned if the bound is a class or extends from a class. This
	 * means that the returned type could be a type variable too.
	 */
	@NullableDecl
	 ...
	 * This method is similar but different from
	 * {@link Class#getGenericInterfaces()}. For example, {@code new
	 * TypeToken<List<String>>() {}.getGenericInterfaces()} will return a list that
	 * contains {@code new TypeToken<Iterable<String>>() {}}; while
	 * {@code List.class.getGenericInterfaces()} will return an array that contains
	 * {@code Iterable<T>}, where the {@code T} is the type variable declared by
	 * interface {@code Iterable}.
	 *
	 * <p>
	 * If this type is a type variable or wildcard, its upper bounds are examined
	 * and those that are either an interface or upper-bounded only by interfaces
	 * are returned. This means that the returned types could include type variables
	 * too.
	 */
	...
	/**
	 * Returns the set of interfaces and classes that this type is or is a subtype
	 * of. The returned types are parameterized with proper type arguments.
	 *
	 * <p>
	 * Subtypes are always listed before supertypes. But the reverse is not true. A
	 * type isn't necessarily a subtype of all the types following. Order between
	 * types without subtype relationship is arbitrary and not guaranteed.
	 *
	 * <p>
	 ...
	 * {@code
	 * ArrayList<String>}, {@code Iterable<String>} is returned given the input
	 * {@code
	 * Iterable.class}.
	 */
	 ...
	 * example, if this is {@code Iterable<String>} and {@code subclass} is
	 * {@code List}, {@code List<String>} is returned.
	 */
	 ...
	 * "Supertype" is defined according to <a href=
	 * "http://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.5.1">the
	...
	/**
	 * Returns true if this type is a subtype of the given {@code type}. "Subtype"
	 * is defined according to <a href=
	 * "http://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.5.1">the
	 * rules for type arguments</a> introduced with Java generics.
	 *
	 * @since 19.0
	...
	public final boolean isSubtypeOf(Type supertype) {
		...
		// if 'this' is wildcard, it's a suptype of to 'supertype' if any of its
		// "extends"
	 ...
	 * {@code int[]}, {@code T[]}, {@code <? extends Map<String, Integer>[]>} etc.
	 */
	 ...
	 * ({@code int[]}, {@code T[]}, {@code <? extends Map<String, Integer>[]>}
	 * etc.), or else {@code null} is returned.
	 */
	@NullableDecl
	...
	/**
	 * Returns the {@link Invokable} for {@code method}, which must be a member of
	 * {@code T}.
	 *
	 * @since 14.0
	 */
	...
	/**
	 * Returns the {@link Invokable} for {@code constructor}, which must be a member
	 * of {@code T}.
	 *
	 * @since 14.0
	 */
	...
	public class TypeSet implements Serializable {
		...
		protected Set<TypeToken<? super T>> delegate() {
			...
			if (filteredTypes == null) {
				...
				return (types = collectedTypes.stream().filter(TypeFilter.IGNORE_TYPE_VARIABLE_OR_WILDCARD)
		...
		public Set<Class<? super T>> rawTypes() {
			...
			// Class.
			@SuppressWarnings({ "unchecked", "rawtypes" })
			List<Class<? super T>> collectedTypes = (List) TypeCollector.FOR_RAW_TYPE.collectTypes(getRawTypes());
	...
	private final class InterfaceSet extends TypeSet {
		...
		protected Set<TypeToken<? super T>> delegate() {
			...
			if (result == null) {
				return (interfaces = allTypes.delegate().stream().filter(TypeFilter.INTERFACE_ONLY)
						.collect(Collectors.toSet()));
			} else {
				return result;
			}
		...
		public Set<Class<? super T>> rawTypes() {
			...
			@SuppressWarnings({ "unchecked", "rawtypes" })
			List<Class<? super T>> collectedTypes = (List) TypeCollector.FOR_RAW_TYPE.collectTypes(getRawTypes());
			return collectedTypes.stream().filter(new Predicate<Class<?>>() {
				@Override
	...
	private final class ClassSet extends TypeSet {
		...
		protected Set<TypeToken<? super T>> delegate() {
			...
			if (result == null) {
				@SuppressWarnings({ "unchecked", "rawtypes" })
				List<TypeToken<? super T>> collectedTypes = (List) TypeCollector.FOR_GENERIC_TYPE.classesOnly()
						.collectTypes(TypeToken.this);
				return (classes = collectedTypes.stream().filter(TypeFilter.IGNORE_TYPE_VARIABLE_OR_WILDCARD)
						.collect(Collectors.toSet()));
			} else {
				return result;
			}
	...
	/**
	 * Returns true if {@code o} is another {@code TypeToken} that represents the
	 * same {@link Type}.
	 */
	@Override
	 ...
	 * {@code Foo}.
	 * </ol>
	 * Note that condition 2 isn't technically accurate under the context of a
	 * recursively bounded type variables. For example,
	 * {@code Enum<? extends Enum<E>>} canonicalizes to {@code Enum<?>} where
	 * {@code E} is the type variable declared on the {@code Enum} class
	 * declaration. It's technically <em>not</em> true that
	 * {@code Foo<Enum<? extends Enum<E>>>} is a subtype of {@code Foo<Enum<?>>}
	 * according to JLS. See testRecursiveWildcardSubtypeBug() for a real example.
	 *
	 * <p>
	 * It appears that properly handling recursive type bounds in the presence of
	 * implicit type bounds is not easy. For now we punt, hoping that this defect
	 * should rarely cause issues in real code.
	 *
	 * @param formalType
	 *            is {@code Foo<formalType>} a supertype of {@code Foo<T>}?
	 * @param declaration
	 *            The type variable in the context of a parameterized type. Used to
	 *            infer type bound when {@code formalType} is a wildcard with
	...
	/**
	 * In reflection, {@code Foo<?>.getUpperBounds()[0]} is always
	 * {@code Object.class}, even when Foo is defined as
	 * {@code Foo<T extends String>}. Thus directly calling
	 * {@code <?>.is(String.class)} will return false. To mitigate, we canonicalize
	 * wildcards by enforcing the following invariants:
	 * <ol>
	 * <li>{@code canonicalize(t)} always produces the equal result for equivalent
	 * types. For example both {@code Enum<?>} and {@code Enum<? extends Enum<?>>}
	 * canonicalize to {@code Enum<? extends Enum<E>}.
	 * <li>{@code canonicalize(t)} produces a "literal" supertype of t. For example:
	 * {@code Enum<? extends Enum<?>>} canonicalizes to {@code Enum<?>}, which is a
	 * supertype (if we disregard the upper bound is implicitly an Enum too).
	...
	// WARNING: the returned type may have empty upper bounds, which may violate
	// common expectations
	// by user code or even some of our own code. It's fine for the purpose of
	// checking subtypes.
	...
	private Set<Class<? super T>> getRawTypes() {
		final Set<Class<?>> builder = new HashSet<>();
		new TypeVisitor() {
			@Override
	...
	private boolean isOwnedBySubtypeOf(Type supertype) {
		for (TypeToken<?> type : getTypes().delegate()) {
	 ...
	 * {@code cls}. For example: {@code TypeToken.getGenericType(Iterable.class)}
	 * returns {@code Iterable<T>}.
	 *
	 * <p>
	 * If {@code cls} isn't parameterized and isn't a generic array, the type token
	...
	static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
		...
		if ((typeParams.length > 0) || ((ownerType != null) && ownerType != cls.getEnclosingClass())) {
			...
			TypeToken<? extends T> type = (TypeToken<? extends T>) of(
					Types.newParameterizedTypeWithOwner(ownerType, cls, typeParams));
			return type;
		} else {
			return of(cls);
		}
	...
	private abstract static class TypeCollector<K> {
		...
		final List<K> collectTypes(K type) {
			return collectTypes(Arrays.asList(type));
		}
		...
		List<K> collectTypes(Iterable<? extends K> types) {
			...
			return sortKeysByValue(map, Collections.reverseOrder());
		}
		...
		private static <K, V> List<K> sortKeysByValue(final Map<K, V> map,
				final Comparator<? super V> valueComparator) {
			Comparator<K> keyOrdering = new Comparator<K>() {
				@Override
			...
			Collections.sort(list, keyOrdering);
			return list;
		}
